...
Сервис рассылок забирает из 1С товары, состав корзины и статусы заказов.
...
* База: `https://1c.groster.me/base/hs/leadhit/v1` (путь уточнить)
...
`GET /cart?session_id=s_7f3a91c2` (или `?user_id=...`, `?email=...`)

```json
{
  "session_id": "s_7f3a91c2",
  "user_id": "42817",
  "email": "m.orlova@mail.ru",
  "is_empty": false,
  "cart_items": [
    { "product_id": "0057412", "category_id": "salfetki-bumajnye", "name": "Салфетка бумажная \"Лилия\" белая, 250 лист", "price": 71.60, "unit": "пач", "qty": 2 },
    { "product_id": "0038921", "category_id": "kasaletki", "name": "Касалетка 900 мл алюминиевая", "price": 11.80, "unit": "шт", "qty": 100 }
  ],
  "cart_hash": "7c1e9a2f4b",
  "updated_at": "2026-07-23T09:05:00+03:00"
}
```

`user_id` и `email` могут быть пустыми (аноним). При пустой корзине `is_empty: true`.
...
1. Прокидывается ли `session_id` из cookie сайта в 1С? От этого зависит, как находить корзину анонима.
